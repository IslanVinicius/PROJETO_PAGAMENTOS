...
function OrcamentoCadastro() {
    ...
    const handleGerarPDF = async () => {
        ...
        setLoading(true);
        try {
            // Buscar dados completos da empresa, prestador e dados bancários
            const [empresaData, prestadorData] = await Promise.all([
                empresaService.obter(empresaID),
                prestadorService.obter(idPrestador)
            ]);

            // Buscar dados bancários do prestador
            let dadosBancarios = null;
            try {
                const response = await fetch(`${localStorage.getItem('api_url') || 'http://localhost:8080/api'}/dados-bancarios/prestador/${idPrestador}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
                if (response.ok) {
                    dadosBancarios = await response.json();
                }
            } catch (error) {
            ...
            // Tentar carregar logo da empresa (se existir)
            try {
                // Logo placeholder - você pode substituir por uma URL real
                // const logoUrl = '/logo.png'; // Caminho para logo na pasta public
                // const imgData = await fetch(logoUrl).then(r => r.blob());
                // const base64 = await new Promise(resolve => {
                //     const reader = new FileReader();
                //     reader.onloadend = () => resolve(reader.result.split(',')[1]);
                //     reader.readAsDataURL(imgData);
                // });
                // doc.addImage(base64, 'PNG', margin, yPos, 40, 20);
            } catch (error) {
                console.log('Logo não disponível');
            }

            // Título principal alinhado à direita
            doc.setFontSize(24);
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(40, 40, 40);
            doc.text('ORÇAMENTO', pageWidth - margin - 40, yPos, { align: 'right' });
            yPos += 10;
            ...
            doc.text(`Nº ${orcamentoID}`, pageWidth - margin - 40, yPos, { align: 'right' });
            yPos += 15;
            ...
            // ============================================
            // DADOS DA EMPRESA (PRESTADOR DE SERVIÇOS)
            // ============================================
            ...
            // Dados da empresa em formato de tabela
            const empresaLines = [
                `Razão Social: ${empresaData.razao || 'N/A'}`,
                `CNPJ: ${empresaData.cnpj || 'N/A'}`,
                `Nome Fantasia: ${empresaData.nome || 'N/A'}`
            ];
            ...
            // Endereço da empresa
            if (empresaData.enderecos && empresaData.enderecos.length > 0) {
                const endereco = empresaData.enderecos[0]; // Primeiro endereço
                doc.text(`Endereço: ${endereco.logradouro}, ${endereco.numero}${endereco.complemento ? ' - ' + endereco.complemento : ''}`, margin, yPos);
                yPos += 6;
                doc.text(`Bairro: ${endereco.bairro} - ${endereco.cidade}/${endereco.estado}`, margin, yPos);
                yPos += 6;
                doc.text(`CEP: ${endereco.cep}`, margin, yPos);
                yPos += 10;
            } else {
                yPos += 5;
            }
            ...
            const clienteLines = [
                `Nome: ${prestadorData.nome || 'N/A'}`,
                `CPF/CNPJ: ${prestadorData.cpf || 'N/A'}`
            ];
            ...
            if (dadosBancarios) {
                ...
                if (dadosBancarios.banco) {
                    doc.text(`Banco: ${dadosBancarios.banco}`, margin + 5, yPos);
                    yPos += 5;
                }
                if (dadosBancarios.agencia) {
                    doc.text(`Agência: ${dadosBancarios.agencia}`, margin + 5, yPos);
                    yPos += 5;
                }
                if (dadosBancarios.conta) {
                    doc.text(`Conta: ${dadosBancarios.conta}`, margin + 5, yPos);
                    yPos += 5;
                }
                if (dadosBancarios.chavePix) {
                    doc.text(`Chave PIX: ${dadosBancarios.chavePix}`, margin + 5, yPos);
                    yPos += 5;
                }
                if (dadosBancarios.tipoConta) {
                    doc.text(`Tipo de Conta: ${dadosBancarios.tipoConta}`, margin + 5, yPos);
                    yPos += 5;
                }
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(0, 51, 102);
            doc.text('DADOS DO ORÇAMENTO', margin + 5, yPos + 3);
            yPos += 15;
            ...
            // Informações do orçamento em colunas
            ...
            if (tipoPagamento) {
                ...
                doc.text(`Pagamento: ${tipoPagamentoText}`, margin, yPos);
                yPos += 7;
            }
            
            // Coluna 2 (data de emissão)
            const dataEmissao = new Date().toLocaleDateString('pt-BR');
            doc.text(`Emissão: ${dataEmissao}`, margin + colWidth, yPos - 7);
            yPos += 10;
            ...
            if (descricao) {
                ...
                yPos += (descricaoLines.length * 6) + 10;
            }

            // ============================================
            // ITENS DO ORÇAMENTO (TABELA)
            // ============================================
            ...
            if (itensOrcamento.length > 0) {
                ...
                const colItem = tableWidth * 0.45;      // 45% para item
                const colQtd = tableWidth * 0.15;       // 15% para quantidade
                const colUnit = tableWidth * 0.20;      // 20% para valor unitário
                const colTotal = tableWidth * 0.20;     // 20% para valor total
                ...
                doc.rect(margin, yPos - 5, colItem, 8, 'F');
                doc.rect(margin + colItem, yPos - 5, colQtd, 8, 'F');
                doc.rect(margin + colItem + colQtd, yPos - 5, colUnit, 8, 'F');
                doc.rect(margin + colItem + colQtd + colUnit, yPos - 5, colTotal, 8, 'F');
                
                doc.text('ITEM', margin + 3, yPos);
                doc.text('QTD', margin + colItem + (colQtd / 2), yPos, { align: 'center' });
                doc.text('VALOR UNITÁRIO', margin + colItem + colQtd + (colUnit / 2), yPos, { align: 'center' });
                doc.text('VALOR TOTAL', margin + colItem + colQtd + colUnit + (colTotal / 2), yPos, { align: 'center' });
                ...
                itensOrcamento.forEach((item, index) => {
                    ...
                    const itemNome = item.itemNome.length > 40 ? item.itemNome.substring(0, 37) + '...' : item.itemNome;
                    
                    doc.text(itemNome, margin + 3, yPos);
                    doc.text(String(item.quantidade), margin + colItem + (colQtd / 2), yPos, { align: 'center' });
                    doc.text(formatarValor(item.valorUnitario), margin + colItem + colQtd + (colUnit / 2), yPos, { align: 'right' });
                    doc.text(formatarValor(item.valorTotal), margin + colItem + colQtd + colUnit + (colTotal / 2) - 3, yPos, { align: 'right' });
                    yPos += 6;
                });
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(0, 51, 102);
            doc.text('RESUMO FINANCEIRO', summaryX + 5, yPos);
            yPos += 10;
            ...
            // Total dos itens
            doc.text('Total dos Itens:', summaryX, yPos);
            doc.text(formatarValor(valorTotalItens), summaryX + summaryWidth - 5, yPos, { align: 'right' });
            yPos += 8;
            
            // Desconto (se houver)
            if (desconto && parseFloat(desconto) > 0) {
                doc.text('Desconto:', summaryX, yPos);
                doc.text('- ' + formatarValor(desconto), summaryX + summaryWidth - 5, yPos, { align: 'right' });
                yPos += 8;
            }
            ...
            doc.text('TOTAL GERAL:', summaryX, yPos);
            doc.text(formatarValor(valorFinal), summaryX + summaryWidth - 5, yPos, { align: 'right' });
            yPos += 15;
            ...
            const footerText = 'Este orçamento é válido por 30 dias a partir da data de emissão.';
            doc.text(footerText, pageWidth / 2, yPos, { align: 'center' });
            yPos += 10;
            
            doc.text('Documento gerado eletronicamente em ' + dataEmissao, pageWidth / 2, yPos, { align: 'center' });
            yPos += 15;
